...
$("#btnDownloadPDF").addEventListener("click", () => {
  if (!tblResultsBody.children.length || tblResultsBody.querySelector(".table-empty")) {
    alert("⚠️ Run algorithm first before exporting!");
    return;
  }
  ...
  y += 8;
  doc.setFontSize(12);
  doc.text(`Average Turnaround Time: ${avgTAT.value}`, 20, y); y+=7;
  doc.text(`Average Waiting Time: ${avgWT.value}`, 20, y); y+=7;
  doc.text(`Throughput: ${throughput.value}`, 20, y);

  // Gantt Chart (text timeline)
  y += 12;
  doc.setFontSize(14);
  doc.text("Gantt Chart:", 14, y);
  y += 8;
  let timelineText = Array.from(ganttChart.children).map(b => b.innerText).join(" → ");
  doc.setFontSize(11);
  doc.text(timelineText, 20, y, { maxWidth: 170 });

  // Footer with hyperlink
  y = 280;
  doc.setFontSize(11);
  doc.textWithLink("Generated by Harshsfd", 14, y, { url: "https://www.linkedin.com/in/harshsfd" });
...
$("#btnDownloadCSV").addEventListener("click", () => {
  ...
  let rows = [["Process ID","Arrival","Burst","Completion","Waiting","Turnaround"]];
  Array.from(tblResultsBody.children).forEach(tr => {
    let cols = Array.from(tr.children).map(td => td.innerText);
    rows.push(cols);
  });

  rows.push([]);
  rows.push(["Average Turnaround Time", avgTAT.value]);
  rows.push(["Average Waiting Time", avgWT.value]);
  rows.push(["Throughput", throughput.value]);
  rows.push([]);
